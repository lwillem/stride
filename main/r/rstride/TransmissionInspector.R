...
inspect_transmission_dynamics <- function(project_dir,save_pdf = TRUE)
{
  ...
  for(i_config in 1:nrow(input_opt_design)){
    ...
    ## DOUBLING TIME ----
    # make sure the different stochastic runs are not connected
    data_incidence$doubling_time[data_incidence$sim_date == min(data_incidence$sim_date,na.rm=T)] <- NA
    # make sure the extreme values (outside the plot area), are excluded
    plot_ylim <- c(0,14)
    data_incidence$doubling_time[data_incidence$doubling_time < min(plot_ylim) | data_incidence$doubling_time > max(plot_ylim)] <- NA
                                        ...
                                        sim_date_range)
{
  ...
  summary_out$doubling_time <- get_longitudinal_doubling_time(summary_out$new_infections)$mean
  ...
  ## LOCATION ----
  pool_type_opt <- c("Household","HouseholdCluster",
                     "CommunityWeekend","CommunityWeekday",
                     "Workplace",'Collectivity','School',
                     'OtherHouse','Transport',
                     'OtherPlace','RestoCafe') # TODO: make this flexible?
  ...
  summary_generation     <- sec_transm[,.(gen_interval = mean(gen_interval,na.rm=T)),by=sim_date]
  summary_out    <- merge(summary_out,summary_generation,all.x = TRUE)
